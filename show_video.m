function update_visualization_func = show_video(frame_name_content, sequence_path)
%SHOW_VIDEO
...
%   http://www.isr.uc.pt/~henriques/
	...
	function stop = update_visualization(n, box)
	...
	function redraw(frame)
		%render main image
		im = imread([sequence_path frame_name_content{frame}]);
		if size(im,3) > 1
			im = rgb2gray(im);
		end
% 		if resize_image,
